var wigzoConf = {
    ...
    "orgtoken": "39ca9237-b79a-44ee-a4d7-cb66f5642410",
    "orgIcon": "https://app.wigzo.com/assets/img/profiles/noimage.jpeg",
    ...
    "failSafeUrl": "https://www.wigzo.com/",
}
    ...
    event.waitUntil(caches.open(cacheName).then(function(cache) {
        fetch('').then(function(response) {
            if (response.status === 200) {
                ...
                console.log('Could not find file: . Please make sure you have entered the correct path. Status Code: ' + response.status);
            }
        });
        return cache;
    }));
...
self.addEventListener('activate', function(event) {
    event.waitUntil(caches.keys().then(function(keyList) {
        return Promise.all(keyList.map(function(key) {
            if (key !== cacheName && key !== dataCacheName) { /*console.log('[ServiceWorker] Removing old cache', key);*/
...
self.addEventListener('fetch', function(e) { /*console.log('[Service Worker] Fetch', e.request.url);*/
    if (e.request.url.indexOf(dataUrl) > -1) { /* * When the request URL contains dataUrl, the app is asking for fresh * data. In this case, the service worker always goes to the * network and then caches the response. This is called the "Cache then * network" strategy */
    ...
    } else { /* * The app is asking for app shell files. In this scenario the app uses the * "Cache, falling back to the network" offline strategy: */
